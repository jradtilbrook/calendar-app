...
Window *createWindow(char *title)
{
    ...
    /* Create hbox */
    // TODO: convert to gtk_grid_new for future proofing
    pane = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0); /*PADDING);*/
    gtk_container_add(GTK_CONTAINER(win->gtkWindow), pane);

    /* Create button container (vbox) on the left */
    win->buttonBox = gtk_button_box_new(GTK_ORIENTATION_VERTICAL);
              ...
              int nInputs, InputProperties *properties, char **inputs)
{
    ...
    // TODO: remove deprecated GTK_STOCK_* constants
